...
# df_person.loc[(df_person['AGE'] < df_person['AGE'].quantile(0.001)) | (df_person['AGE'] > df_person['AGE'].quantile(0.999)),
#               'AGE'] = np.nan
# import seaborn as sea
# sea.distplot(df_person['AGE'])
# show()
...
df_person_emb = pd.read_csv('PERSON_ID_emb.csv')
...
df_person = pd.merge(df_person, df_person_emb, on='PERSON_ID', how='left')
# df_person['PERSON_ID'] = df_person['PERSON_ID'].astype("category")
# df_recruit['RECRUIT_ID'] = df_recruit['RECRUIT_ID'].astype("category")
...
# df_train['EDU_BI'] = df_train['EDU'] .map(lambda x: 1 if x>=0 else 0)
# df_test['EDU_BI'] = df_test['EDU'].map(lambda x: 1 if x>=0 else 0)
...
# df_train['YEARS_BI'] = df_train['YEARS'] .map(lambda x: 1 if x>=0 else 0)
# df_test['YEARS_BI'] = df_test['YEARS'].map(lambda x: 1 if x>=0 else 0)
...
feats = feats + ['PERSON_ID_emb_%d'%i for i in range(8)]
# print(df_test[feats+['RECRUIT_ID', 'PERSON_ID']].groupby(['RECRUIT_ID', 'PERSON_ID']).agg('max'))
...
job_feat_names = ['LABEL']#, 'AGE', 'GENDER']#, 'HIGHEST_EDU', 'WORK_YEARS']
# job_feats = [x + '_job_mean' for x in job_feat_names]
job_feats = []
# [job_feats.extend([x + '_job_mean', x + '_job_std', x + '_job_median']) for x in job_feat_names]
...
# skf = StratifiedKFold(n_splits=fold_num, shuffle=True, random_state=666)
...
for fold, (trn_idx, val_idx) in enumerate(skf.split(df_train[feats], df_train['LABEL'])):
    ...
    n_feats = feats + ['LABEL_person_mean'] + job_feats + ['LABEL_GENDER_mean', 'LABEL_GENDER_std', 'LABEL_AGE_mean', 'LABEL_AGE_std',
              ...
              #['LABEL_'+name+'_mean' for name in ['GENDER', 'AGE']]
...
show()
df_test[['RECRUIT_ID', 'PERSON_ID', 'LABEL']].to_csv(time.strftime('ans/ans_%Y%m%d%H%M.csv'), index=False)
